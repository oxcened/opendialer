...
class RecentsAdapter(
  ...
  private val incoming: Drawable? =
    ContextCompat.getDrawable(context, R.drawable.icon_21)
  private val outgoing: Drawable? =
    ContextCompat.getDrawable(context, R.drawable.icon_16)
  private val missed: Drawable? =
    ContextCompat.getDrawable(context, R.drawable.icon_22)
  private val voicemail: Drawable? =
    ContextCompat.getDrawable(context, R.drawable.icon_09)
  private val blocked: Drawable? =
    ContextCompat.getDrawable(context, R.drawable.icon_18)
  ...
  inner class ViewHolder(inflater: LayoutInflater, private val parent: ViewGroup) :
    ...
    private fun expand(
      ...
      val height = CommonUtils.convertDpToPixels(
        itemHeight,
        parent.context
      )
      val expandedHeight = CommonUtils.convertDpToPixels(
        itemHeight + (call.options.size * optionHeight),
        parent.context
      )
      val elevation = CommonUtils.convertDpToPixels(0f, parent.context)
      val elevationExpanded =
        CommonUtils.convertDpToPixels(4f, parent.context)
      if (animate == true) {
        animator?.cancel()
    ...
    fun bind(currentCall: DialerCall, position: Int) {
      ...
      if (!currentCall.isAnonymous() && PermissionUtils.hasContactsPermission(
          context
        )
      ) {
      ...
      val prettyDate = org.ocpsoft.prettytime.PrettyTime().format(currentCall.date)
      ...
      recyclerViewOptions.adapter =
        CallOptionsAdapter(currentCall.options) {
  ...
  fun setData(calls: List<DialerCall>) {
    ...
    if (oldCalls.isEmpty()) {
      ...
      coroutineScope.launch(Dispatchers.IO) {
        val diffUtil =
          RecentsDiffUtil(oldCalls, calls)
