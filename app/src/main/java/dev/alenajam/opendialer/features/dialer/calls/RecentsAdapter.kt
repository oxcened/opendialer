...
import dev.alenajam.opendialer.util.CircleTransform
...
class RecentsAdapter(
  ...
  private val onOptionClick: (call: DialerCall, option: dev.alenajam.opendialer.model.CallOption) -> Unit,
  ...
  inner class ViewHolder(inflater: LayoutInflater, private val parent: ViewGroup) :
    ...
    private fun expand(
      ...
      val height = CommonUtils.convertDpToPixels(itemHeight, parent.context)
      ...
      val elevationExpanded = CommonUtils.convertDpToPixels(4f, parent.context)
      if (animate == true) {
        animator?.cancel()
